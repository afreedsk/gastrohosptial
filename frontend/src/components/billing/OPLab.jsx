...
export default function OPLab() {
  ...
  const fetchLabRecords = async (opts = {}) => {
    ...
    try {
      ...
      const { data: res } = await api.get('/op-lab', { params })
      setData(res.data || [])
  ...
  const applyFilters = () => fetchLabRecords({ page: 1 })
  ...
  const resetFilters = () => {
    ...
    setSearchQuery('')
    setTimeout(() => fetchLabRecords({ page: 1 }), 0)
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap gap-4 items-end">
        ...
        <div className="flex-1 min-w-[200px]">
          <label className="label">Or search item / any field</label>
          <input
            className="input w-full"
            placeholder="Search item, bill no, phone…"
            value={globalSearch}
      ...
      {!loading && data.length > 0 && (
        <div className="overflow-x-auto">
          ...
          <Pagination
            ...
            total={total}
            perPage={PER_PAGE}
            onPageChange={(p) => fetchLabRecords({ page: p })}
          />
        </div>
      )}
