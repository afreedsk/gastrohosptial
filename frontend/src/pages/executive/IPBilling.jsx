...
export default function IPBilling() {
  ...
  const loadBills = () => api.get('/ip-billing').then((r) => setBills(r.data))
  useEffect(() => {
    api.get('/admissions', { params: { status: 'Admitted' } }).then((r) => setAdmissions(r.data))
    loadBills()
  }, [])
  ...
  const due = +(grandTotal - Number(advanceAdjusted || 0) - Number(paid || 0)).toFixed(2)

  const submit = async (e) => {
    e.preventDefault()
    if (!admissionId) return setError('Select an admitted patient first')
    setError('')
    setSaving(true)
    try {
      await api.post('/ip-billing', {
        admission_id: admissionId, ...charges, discount, tax_percent: taxPercent,
        advance_adjusted: advanceAdjusted, paid_amount: paid,
      })
      setAdmissionId(''); setCharges(initCharges); setDiscount(0); setTaxPercent(0)
      setAdvanceAdjusted(0); setPaid(0)
      loadBills()
    } catch (err) {
  ...
  return (
    <div>
      <PageHeader title="IP Billing" subtitle="Inpatient admission, room, procedure and discharge billing" />
        ...
        <form onSubmit={submit} className="xl:col-span-1">
          <Section title="New IP Bill">
            ...
            <select className="input mb-3" value={admissionId} onChange={(e) => setAdmissionId(e.target.value)}>
              ...
              {admissions.map((a) => (
                <option key={a.id} value={a.id}>{a.admission_no} — {a.patient_name} ({a.room_no}/{a.bed_no})</option>
              ))}
            </select>
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              ...
              <div>
                ...
                <input type="number" min="0" className="input" value={taxPercent} onChange={(e) => setTaxPercent(e.target.value)} />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div>
                ...
                <input type="number" min="0" className="input" value={advanceAdjusted} onChange={(e) => setAdvanceAdjusted(e.target.value)} />
              </div>
              <div>
            ...
            <button className="btn-primary w-full flex items-center justify-center gap-2" disabled={saving}>
              <Receipt size={15} /> {saving ? 'Generating…' : 'Generate IP Bill'}
            </button>
          </Section>
        </form>
          ...
          <Section title="IP Bills">
            <div className="overflow-x-auto">
              <table className="table-base">
                <thead>
                  <tr><th>Bill No</th><th>Admission</th><th>Patient</th><th>Grand Total</th><th>Paid</th><th>Due</th><th>Status</th></tr>
                </thead>
                <tbody>
                  {bills.map((b) => (
                    <tr key={b.id}>
